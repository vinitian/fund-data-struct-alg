...
public class BST {
	...
	public int height(BSTNode n) { //find height of a 
		if (n == null) {
			return -1;
		}
	...
	public void makeBalancedTree(ArrayList<Integer> list, int startindex, int endindex) {
		if (list.size() == 0) {
			return;
		}
		
		if (startindex == endindex) {
			insert(list.get(startindex));
			return;
		}
		
		int cut = Math.floorDiv(startindex + endindex, 2);
		
		insert(list.get(cut));
		makeBalancedTree(list, startindex, cut);
		makeBalancedTree(list, cut+1, endindex);
	...
	public void makeBalancedTree() {
		...
		//treeiterator's next() goes to the next min number, so it's already sorted
		ArrayList<Integer> myarr = new ArrayList<Integer>();
		TreeIterator p = (TreeIterator) findMin();
		myarr.add(p.currentNode.data);
		try {
			while (p.hasNext()) {
				p.next();
				myarr.add(p.currentNode.data);
		...
		makeEmpty();
		makeBalancedTree(myarr, 0, myarr.size()-1);
//		BTreePrinter.printNode(root);
		return;

	}
	
	public ArrayList<BSTNode> inOrderArrayList(BSTNode n) {
		...
		inorder.addAll(inOrderArrayList(n.left));	
		inorder.add(n);
		inorder.addAll(inOrderArrayList(n.right));
