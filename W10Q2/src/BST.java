...
public class BST {
	...
	public void addLeftSubTreeToMin(BST subtree) {
		...
		TreeIterator p1 = (TreeIterator) subtree.findMax();
		int subtreeMax = p1.currentNode.data;
		TreeIterator p2 = (TreeIterator) this.findMin(); //p2 is now at leftmost node
		
		TreeIterator p3 = (TreeIterator) findMin(p2.currentNode.right);
		//leftmost node under p2's right
		int ourMin = p3.currentNode.data;
		
		if (subtreeMax >= ourMin) { // if we add the subtree, it won't be BST no more
			return;
		}
		
		//add subtree
		subtree.root.parent = p2.currentNode;
		p2.currentNode.left = subtree.root;
		size += subtree.size;
